...
if (isset($_GET['video_id'])) {
    ...
    if (!empty(trim($video_id))) {
        $request_url = "https://getsuperembed.link/?video_id=" . urlencode(trim($video_id)) . 
            "&tmdb=" . urlencode($is_tmdb) . 
            "&season=" . urlencode($season) . 
            "&episode=" . urlencode($episode) . 
            "&player_font=" . urlencode($player_font) . 
            "&player_bg_color=" . urlencode($player_bg_color) . 
            "&player_font_color=" . urlencode($player_font_color) . 
            "&player_primary_color=" . urlencode($player_primary_color) . 
            "&player_secondary_color=" . urlencode($player_secondary_color) . 
            "&player_loader=" . urlencode($player_loader) . 
            "&preferred_server=" . urlencode($preferred_server) . 
            "&player_sources_toggle_type=" . urlencode($player_sources_toggle_type);
        ...
        if (!empty($player_url)) {
            if (strpos($player_url, "https://") !== false) {
                // Forward the seek parameters t and start if they are present
                $t = isset($_GET['t']) ? $_GET['t'] : null;
                $start = isset($_GET['start']) ? $_GET['start'] : null;
                
                if ($t !== null || $start !== null) {
                    $parts = parse_url($player_url);
                    $query = [];
                    ...
                    // Reconstruct player URL
                    $player_url = (isset($parts['scheme']) ? $parts['scheme'] . '://' : '')
                ...
                echo "<span style='color:red'>" . htmlspecialchars($player_url) . "</span>";
            }
        } else {
