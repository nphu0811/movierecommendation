...
class MetadataRepairServiceTest {
    ...
    @Test
    void whenDuplicatePostersExist_deletesAndRepairs() throws Exception {
        // Setup two movies sharing the same poster:
        // Movie 1: Inception (correct poster)
        Movie movie1 = new Movie();
        movie1.setMovieId(10);
        movie1.setTitle("Inception");
        movie1.setReleaseYear(2010);
        movie1.setPosterUrl("https://image.tmdb.org/t/p/w342/inception.jpg");

        // Movie 2: Hellboy (incorrect duplicate poster)
        Movie movie2 = new Movie();
        movie2.setMovieId(11);
        movie2.setTitle("Hellboy");
        movie2.setReleaseYear(2004);
        movie2.setPosterUrl("https://image.tmdb.org/t/p/w342/inception.jpg");

        when(movieRepository.findAll()).thenReturn(List.of(movie1, movie2));

        // Mock TMDB search result for "Inception"
        Map<String, Object> searchInception = Map.of(
            "results", List.of(
                Map.of(
                    "id", 27205,
                    "title", "Inception",
        ...
        // Mock TMDB search result for "Hellboy"
        Map<String, Object> searchHellboy = Map.of(
            "results", List.of(
                Map.of(
                    "id", 7373,
                    "title", "Hellboy",
                    "release_date", "2004-04-02",
                    "overview", "Hellboy overview"
                )
            )
        );

        // Details for Inception
        Map<String, Object> detailsInception = Map.of(
            "id", 27205,
            "title", "Inception",
            "release_date", "2010-07-15",
            "poster_path", "/inception.jpg",
        ...
        // Details for Hellboy
        Map<String, Object> detailsHellboy = Map.of(
            "id", 7373,
            "title", "Hellboy",
            "release_date", "2004-04-02",
            "poster_path", "/hellboy.jpg",
            "backdrop_path", "/hellboy_backdrop.jpg",
            "overview", "Hellboy overview"
        );
        ...
        // RestTemplate behavior
        when(restTemplate.getForObject(containsString("query=Inception"), eq(Map.class))).thenReturn(searchInception);
        when(restTemplate.getForObject(containsString("query=Hellboy"), eq(Map.class))).thenReturn(searchHellboy);
        when(restTemplate.getForObject(containsString("/movie/27205"), eq(Map.class))).thenReturn(detailsInception);
        when(restTemplate.getForObject(containsString("/movie/7373"), eq(Map.class))).thenReturn(detailsHellboy);

        // Run
        repairService.repairDuplicatePosters();

        // Verify: both should have deleteLink called to clear old link, and then updated with correct info
        verify(repairHelper).deleteLink(10);
        verify(repairHelper).deleteLink(11);

        verify(repairHelper).updateMovieMetadata(
            eq(10), eq(27205),
            eq("https://image.tmdb.org/t/p/w342/inception.jpg"),
            eq("https://image.tmdb.org/t/p/w780/inception_backdrop.jpg"),
            eq("Inception overview"),
            isNull()
        );

        verify(repairHelper).updateMovieMetadata(
            eq(11), eq(7373),
            eq("https://image.tmdb.org/t/p/w342/hellboy.jpg"),
            eq("https://image.tmdb.org/t/p/w780/hellboy_backdrop.jpg"),
            eq("Hellboy overview"),
            isNull()
        );
    }
